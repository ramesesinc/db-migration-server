...
class MsSqlProvider extends DBProvider {
  ...
  extractKeyValue(value) {
    const keyValueArr = [];
    for (let key in value) {
      if (value.hasOwnProperty(key)) {
        keyValueArr.push(key);
        keyValueArr.push(value[key]);
      }
    }
    return keyValueArr;
  }
  ...
  execute(sql, values = []) {
    return new Promise((resolve, reject) => {
      ...
      for (let i = 0; i < values.length; i++) {
        const [key, value] = this.extractKeyValue(values[i]);
        request.input(key, this._mssql.VarChar, value);
      }
