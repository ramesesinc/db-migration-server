...
class SqlBuilder {
  ...
  serializeValue(field, value) {
    ...
    const serialize = this.serializer[dataType.toLowerCase()];
    return serialize(field, value);
  }

  deserializeValue(field, value) {
    const dataType = field.type || "string";
    const deserialize = this.deserializer[dataType.toLowerCase()];
  ...
  getUpdateFields(data) {
    ...
    for (let key in data) {
      ...
      updateFields.push(
        this.delimitField(field.name) + "=" + this.getValuePlaceholder(field.name)
      );
      values.push({
        field: field.name, 
